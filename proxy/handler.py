...
async def handle_connection(browser_reader, browser_writer):
    ...
    try:
        ...
        if USE_QVPN_TUNNEL:
            ...
            # Tell the QVPN client where to forward this connection
            dest_writer.write(f"DEST {dest_host}:{dest_port}\n".encode())
            await dest_writer.drain()
            # Wait for QVPN client to confirm the encrypted gateway tunnel is ready
            ack = await dest_reader.readline()
            if ack.strip() != b"READY":
                raise ConnectionError(f"QVPN tunnel setup failed for {dest_host}:{dest_port}")
        else:
