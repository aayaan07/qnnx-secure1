...
try:
    # Only attempt to import Windows-specific modules on Windows
    if os.name == "nt":
    ...
    # ImportError or other issues — treat as unavailable
    _WIN32_AVAILABLE = False
        ...
        for proc in psutil.process_iter(["pid", "name", "ppid"]):
            try:
                ...
                try:
                    ...
                    # escalate (the old code defaulted to CRITICAL here, which
                    # caused most false positives, especially when un-elevated).
                    severity = None
...
# ---------------------------------------------------------------------------
class USBMonitor:
    def __init__(self, on_insert: Callable[[str], None], emit_alert: Callable):
    ...
    def start(self):
        if not _WMI_AVAILABLE:
            ...
            self._emit_alert("MONITOR_DEGRADED", "WARNING", "WMI not available — USB monitoring disabled.")
            return
        self._stop.clear()
...
class WindowsLogMonitor:
    def __init__(self, on_unknown_process: Callable[[str], None], on_port_scan: Callable, emit_alert: Callable):
    ...
    def start(self):
        if not _WIN32_AVAILABLE:
            ...
            self._emit_alert("MONITOR_DEGRADED", "WARNING", "win32evtlog not available — Windows log monitoring disabled.")
            return
    ...
    def _process_event(self, event):
        ...
        if event_id == 4688:  # Process creation
            if event.StringInserts and len(event.StringInserts) > 5:
                ...
                is_allowed = any(allowed in path for allowed in ALLOWED_BACKGROUND_PROCESSES)
                if not is_allowed and "c:\\windows\\" not in path and "c:\\program files" not in path:
                    self._on_process(path)

        elif event_id == 5152:  # Firewall packet drop
            self._port_drops += 1
            if self._port_drops > 500:
                self._on_port_scan()
                self._port_drops = 0
...
class QVPNThreatEngine:
    ...
    def __init__(self, store: SQLiteStore, pusher: GatewayAlertPusher):
        ...
        self._usb       = USBMonitor(on_insert=self._on_usb_insert, emit_alert=self.emit_alert)
